...
function handleGet(connection,req,res) {
  ...
  //var query_param = [(page_no-1), page_size];
  var query_param = [];
  if (q !== '') query_param.unshift('%'+q+'%');
  log.info('[moneybook] handleGet() query_param='+query_param+' ['+query_param.length+']');

  // total count
  var count_query_str = 'SELECT COUNT(id) count FROM moneybooks';
  if (q !== '') query_str += ' WHERE item LIKE ?';
  var total_count = 0;
  connection.query(count_query_str, query_param, function handleSql(err, row){
    log.info('[moneybook] handleGet() select count... row='+JSON.stringify(row));
    total_count = row[0].count;
  });

  var start_index = (page_no-1) * page_size;
  log.info('[moneybook] handleGet() start_index='+start_index);
  ...
  */
  ...
  if (q !== '') query_str += ' WHERE item LIKE ?';
  query_str += ' ORDER BY '+sort_type+' '+sort_order+' LIMIT '+start_index+','+page_size;
  log.info('[moneybook] handleGet() query_str='+query_str);


  connection.query(query_str, query_param, function handleSql(err, rows) {
    if (err){ logAndRespond(err,res); return; }
    //if (rows.length === 0){ res.send(204); return; }
    log.info('[moneybook] handleGet() total_count='+total_count+', rows='+JSON.stringify(rows));
    res.send({
      status: 'ok',
      msg: '',
      results: {
        total_count: total_count,
        page_count: parseInt(total_count / page_size),
        page_size : page_size,
        data: rows
      }
    });
...
function handleStateMonths(connection,req,res) {
  ...
  query_str += ' ORDER BY '+sort_type+' '+sort_order+' LIMIT '+(page_no-1)+','+page_size;
  log.debug('[moneybook] handleStateMonths() query_str='+query_str);

  var query_param = [];
  if (q !== '') query_param.unshift('%'+q+'%');
  log.debug('[moneybook] handleStateMonths() query_param='+query_param+' ['+query_param.length+']');
