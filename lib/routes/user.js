...
function handlePost(connection,req,res) {
  ...
  if (count < 1) {
    connection.query('INSERT INTO members SET ?', param, function handleSql(err, result) {
      ...
      var query_str_getid = 'SELECT id FROM members WHERE email = ?';
      var query_param_getid = [];
      query_param_getid.unshift(param.email);
      var count_getid;
      connection.query(query_str_getid, query_param_getid, function handleSql(err, row_getid){
        count_getid = row_getid[0].id;
        log.info('[user] handlePost count_getid='+count_getid);
        res.cookie('auth', count_getid); // member_id
//        connection.release();
      });
...
function handlePut(connection,req,res) {
  log.debug('req.bogy.email: '+req.body.email);
  //log.debug(req.email);
  var param = {
    email: req.body.email,
...
//  log.info(res.cookie('auth'));
  log.debug('***********************************');
  connection.query('UPDATE members SET email=\''+param.email+'\', password=\''+param.new_password+'\' WHERE id='+users_id+' and password=\''+param.prev_password+'\'', function handleSql(err) {
    if (err){ logAndRespond(err,res); return; }
    res.send({
      status: 'ok',
      msg: '',
